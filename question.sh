...
#
sed 's/\,/\./' $1 | sed 's/\(\/[0-9][0-9]\/\)/\120/' | sed 's/\([0-9]\)\/\([0-9]\)\/\([0-9][0-9][0-9][0-9]\)/0\1\/0\2\/\3/g' | sed 's/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9][0-9][0-9]\)/\3-\2-\1/g' | sed 's/\([a-z]\)/\U\1/'| sed 's/;N/\;\N/g' |sed 's/\;\;/\;\\N\;/g' | sed -e's/\;.$/\;\\N/g'| sed 's/\;N/\;\\N/g' | sed 's/\;/\,/g'